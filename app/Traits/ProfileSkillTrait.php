...
trait ProfileSkillTrait
{
        ...
        if (isset($user) && count($user->profileSkills)):
            foreach ($user->profileSkills as $skill):
                ...
                $html .= '<tr id="skill_' . $skill->id . '">
						...
						<td><a href="javascript:;" onclick="showProfileSkillEditModal(' . $skill->id . ');" class="text text-warning">' . __('Edit') . '</a>&nbsp;|&nbsp;<a href="javascript:;" class="delete-btn-skills text text-danger" data-uuid="'.$skill->id.'">' . __('Delete') . '</a></td>
								</tr>';
            endforeach;
        endif;
    ...
    public function deleteProfileSkillPersion($id)
    {
        try {
            $profileSkill = ProfileSkill::findOrFail($id);
            $profileSkill->delete();
            ...
            $success = true;
            $message = "Successfully to Delete this Profile Skill.";
        } catch (\Exception $e) {
            $message = "Failed to Delete this Profile Skill.";
            $success = false;
        }
            if($success == true) {
