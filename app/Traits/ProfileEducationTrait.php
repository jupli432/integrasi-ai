...
trait ProfileEducationTrait
{
        ...
        if (isset($user) && count($user->profileEducation)):
            foreach ($user->profileEducation as $education):
                ...
                $html .= '<div class="col-lg-6">
						...
						<div class="panel-footer mt-3"><a href="javascript:void(0);" onclick="showProfileEducationEditModal(' . $education->id . ',' . $education->state_id . ',' . $education->city_id . ',' . $education->degree_type_id . ');" class="text text-default">' . __('Edit') . '</a>&nbsp;|&nbsp;<a href="javascript:;" class="delete-btn-educations text text-danger" data-uuid="'.$education->id.'">' . __('Delete') . '</a></div>
						</div></div>';
            endforeach;
        endif;
    ...
    public function deleteProfileEducationPersion($id)
    {
        try {
            $profileEducation = ProfileEducation::findOrFail($id);
            ProfileEducationMajorSubject::where('profile_education_id', '=', $id)->delete();
            ...
            $success = true;
            $message = "Successfully to Delete this Profile Education.";
        } catch (\Exception $e) {
            $message = "Failed to Delete this Profile Education.";
            $success = false;
        }
            if($success == true) {
