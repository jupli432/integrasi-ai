...
class CompaniesController extends Controller
{
    ...
    {
        $selectedJobName = $request->input('jobName', '');
        if (
            ...
            $JobNames = Job::where('company_id', $companyId)
                ->distinct()
                ->pluck('jobs.title');


            $jobApplications = JobApply::whereHas('job.company', function ($query) use ($companyId) {
                ...
                ->with([
                    ...
                    'user.city',
                    'user.jobExperience',
                    'user.latestProfileExperience',
                ])
                ->get();

            $results = $jobApplications->map(function ($application) {
                return (object) [
                    'id' => $application->id,
                    ...
                    'title' => optional($application->job)->title,
                    'created_at' => $application->created_at,
                    'cv_file' => optional($application->profileCv)->cv_file,
                    'lastOnline' => $application->user->lastOnlineDisplay(),


                    'user_image' => $application->user->image,
                    'status' => $application->status,
                    'job_name' => optional($application->job)->title,
                    'gender' => optional($application->user->gender)->name,
                    'age' => Carbon::parse($application->user->date_of_birth)->age,
                    'mobile_num' => $application->user->mobile_num,
                    'email' => $application->user->email,
                    'job_experience' => optional($application->user->getJobExperience())->job_experience,
                    'last_position' => optional($application->user->latestProfileExperience)->title,
                    'state_name' => optional($application->user->state)->state,
                    'city_name' => optional($application->user->city)->city,
                ];
            });
            return view('job.list_candidate', compact(
                'results',
                'JobNames',
                'selectedJobName'
            ));
            ...
            //     'users.expected_salary AS expectedSalary',
            //     'users.salary_currency AS salaryCurrency',
            //     'jobs.title',
            //     'job_apply.created_at',
            //     'profile_cvs.cv_file'
            // )->get();
            // return view('job.list_candidate',
            //     compact(
            //         'results',
            //         'JobNames',
            //         'jobNameFilter'
            ...
            return FacadesRedirect::route('login');
        }
    }
}
